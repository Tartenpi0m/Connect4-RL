...
class Connect4Env(gym.Env):
    ...
    def check_for_episode_termination(self, movecol, row):
        winner, reward_vector = self.winner, [self.reward_step, self.reward_step]
        if self.does_move_win(movecol, row):
            winner = 1 - self.current_player
            if winner == 0: reward_vector = [self.reward_win, -self.reward_win]
            elif winner == 1: reward_vector = [-self.reward_win, self.reward_win]
        elif self.get_moves() == []:  # A draw has happened
            winner = -1
            reward_vector = [self.reward_draw, self.reward_draw]
        
        if winner is not None and winner != -1: # A player has won
            winner += 1
